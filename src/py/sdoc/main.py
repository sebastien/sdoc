...
def compact_html(text):
    ...
    body = body.replace("javascript:documentElement", "javascript:dO").replace(
        "javascript:describeElement", "javascript:dS"
    )
...
class Documenter:
    ...
    def __init__(self, modules=None, encoding="utf-8"):
        ...
        if isinstance(modules, str):
            self._acceptedModules.append(modules)
        elif modules:
            self._acceptedModules.extend(modules)
    ...
    def _isExternalValue(self, value):
        ...
        if (
            ...
            and not getattr(value, "__module__") == self._currentModule.__name__
        ):
    ...
    def _keysByType(self, something):
        ...
        for key in self._getAllSlotsNames(something):
            ...
            if key not in self._getOwnSlotsNames(something):
                mod = MOD_INHERITED
            else:
                mod = ""
        ...
        for key, values in result.items():
            # TODO: Should refine the criteria
            values.sort()
        return result
    ...
    def id(self, something):
        ...
        if result == None:
            ...
            val = len(IDS)
            limit = len(string.ascii_letters)
            if val < 0:
                result += "0"
                val = abs(val)
            result = ""
            ...
            while True:
                mod = val % limit
                val = val / limit
                result += string.ascii_letters[mod]
                if val == 0:
                    break
            IDS[the_id] = result
        assert len(RETAIN) == len(IDS.keys()), "%s != %s" % (
            len(RETAIN),
            len(IDS.keys()),
        )
        return result
    ...
    def isSkipped(self, name, value):
        ...
        be skipped."""
        if name.startswith("__") and name not in (SPECIAL_ATTRIBUTES.keys()):
            return True
        if (
    ...
    def representation(self, something):
        ...
        if type(something) in (
            types.FunctionType,
            types.MethodType,
        ):
            return self._describeFunction(something)
        if type(something) in (tuple, list, dict, str):
    ...
    def describe(self, something):
        ...
        if self._contents.get(this_id) != None:
            return self._contents.get(this_id)
        result = [
            f"<div id='{this_id}' class='description'>"
            f"<h1>{self.describeType(something)}</h1>"
            f"<div class='representation'>{self.representation(something)}</div>"
            "<div class='docstring'>"
        ]
        if self._hasDocumentation(something):
            result.append(
                str(
                    html_escape(self._getDocumentation(something)).replace(
                        "\n", "<br />"
                    )
                )
            )
    ...
    def documentModule(self, name):
        ...
        if self._modulesNavigation:
            self._modulesNavigation += " &bull; "
        else:
            self._modulesNavigation = "API : "
        self._modulesNavigation += (
    ...
    def list(self, name, something, level=0):
        ...
        result = ""
        result += "<div id='%s' class='%s'>" % (
            this_id,
            level == 0 and "root" or "container",
        )
        result += (
            "<div class='name'><a href='javascript:describeElement(\"%s\");'>%s</a></div>"
            % (this_id, name)
        )
        ...
        for some_type in KEYS_ORDER:
            ...
            if not attributes and not inherited:
                continue
            ...
            for group in (attributes, inherited):
                ...
                for attribute in group:
                    ...
                    if self.isSkipped(attribute, child):
                        continue
                    ...
                    if not group_printed:
                        result += (
                            "<div class='title'>%s</div class='title'><div class='group'>"
                            % (mod + some_type)
                        )
                        group_printed = True
                    ...
                    try:
                        is_documented = (
                            self._getDocumentation(child)
                            and "documented"
                            or "undocumented"
                        )
                    except:
                        is_documented = False
                    link = 'href=\'javascript:documentElement("%s","%s");\'' % (
                        this_id,
                        child_id,
                    )
                    type_name = self.typeToName(child)
                    prefix = "&sdot;"
                    if type_name in (KEY_CLASS_METHOD, KEY_METHOD):
                        prefix = "&fnof;"
                    if type_name == KEY_FUNCTION:
                        prefix = "&lambda;"
                    if type_name == KEY_CLASS:
                        prefix = "&Tau;"
                    if attribute.upper() == attribute:
                        prefix = "&bull;"
                    if attribute in SPECIAL_ATTRIBUTES:
                        ...
                        if not isinstance(label, str):
                            ...
                            if label == None:
                                continue
                        prefix = "&equiv;"
                        attribute = "<span class='special %s'>%s</span>" % (
                            attribute,
                            label,
                        )
                    ...
                    class='prefix'>%s</span><a %s>%s</a></div>""" % (
                        is_documented and "documented" or "undocumented",
                        prefix,
                        link,
                        attribute,
                    )
    ...
    def typeToName(self, a_value):
        ...
        if a_type == types.ModuleType:
            ...
            return KEY_CLASS
        elif a_type == types.FunctionType:
            return KEY_FUNCTION
        elif a_type in (types.MethodType,):
            return KEY_METHOD
        elif repr(a_value).startswith("<class '"):
            return KEY_CLASS
        else:
            return KEY_VALUE
    ...
    def toHTML(self, title):
        with open(
            os.path.dirname(os.path.abspath(sdoc.__file__)) + "/sdoc.tmpl", "rt"
        ) as template_f:
...
class LambdaFactoryDocumenter(Documenter):
    ...
    def __init__(self, modules=None, encoding="utf-8"):
        ...
        if not lambdafactory:
            raise ImportError(
                "Lambda factory is required: <http://www.ivy.fr/lambdafactory>"
            )
    ...
    def typeToName(self, a_value):
        ...
        if isinstance(a_value, lif.IModule):
            return KEY_MODULE
        elif isinstance(a_value, lif.IClass):
            return KEY_CLASS
        elif isinstance(a_value, lif.IConstructor):
            return KEY_CONSTRUCTOR
        elif isinstance(a_value, lif.IClassMethod):
            return KEY_METHOD
        elif isinstance(a_value, lif.IMethod):
            return KEY_METHOD
        elif isinstance(a_value, lif.IFunction):
            return KEY_FUNCTION
        elif isinstance(a_value, lif.IClassAttribute):
            return KEY_CLASS_ATTRIBUTE
        elif isinstance(a_value, lif.IAttribute):
...
def run(args):
    ...
    oparser = OptionParser(
        prog="sdoc", description=DESCRIPTION, usage=USAGE, version="SDoc " + __version__
    )
    oparser.add_option(
        "-p", "--path", action="append", dest="pythonpath", help=OPT_PYTHONPATH
    )
    oparser.add_option(
        "-a", "--accepts", action="append", dest="accepts", help=OPT_ACCEPTS
    )
    oparser.add_option(
        "-c", "--compact", action="store_true", dest="compact", help=OPT_COMPACT
    )
    oparser.add_option("-b", "--body", action="store_true", dest="body", help=OPT_BODY)
    oparser.add_option("-t", "--title", dest="title", help=OPT_TITLE)
    oparser.add_option(
        "-e", "--encoding", dest="encoding", default="utf-8", help=OPT_ENCODING
    )
